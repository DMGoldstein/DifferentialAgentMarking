...
fit2.train <- brm(formula = AGENT.EXPONENCE ~ ASPECT + AGENT.NOMINALITY +  SUBJECT.ANIMACY + PARTICIPLE + (1|LEMMA), data = data.train, family = bernoulli(), 
            prior = get_prior(AGENT.EXPONENCE ~ ASPECT + AGENT.NOMINALITY +  SUBJECT.ANIMACY + PARTICIPLE + (1|LEMMA), data = data.train, family = bernoulli()), warmup = 4000, iter = 20000, chains = 6, cores = 6, control = list(adapt_delta = 0.95), save_all_pars = TRUE, seed = 100)
...
fit3.full.random <- brm(formula = AGENT.EXPONENCE ~ ASPECT + AGENT.STATUS + SUBJECT.ANIMACY  + (1 + ASPECT + AGENT.STATUS + SUBJECT.ANIMACY|LEMMA),
            data = hdt.passive, family = bernoulli(),
            prior = get_prior(AGENT.EXPONENCE ~ ASPECT +  AGENT.STATUS + SUBJECT.ANIMACY  + (1|LEMMA), data = hdt.passive, family = bernoulli()),
            warmup = 4000, iter = 20000, chains = 6, cores = 6,
            control = list(adapt_delta = 0.95, max_treedepth = 15), save_all_pars = TRUE)

stanplot(fit3.full.random, 
         type = "areas",
         prob = 0.95)

fit3.ran.effect <- ranef(fit3.full.random)

r_Intercept <- fit3.ran.effect$LEMMA[, , 1] %>%
  as_tibble() %>%
  rownames_to_column(var = "LEMMA") %>%
  mutate(Variable = "sd(Intercept)")

r_Intercept$LEMMA <- rownames(fit3.ran.effect$LEMMA)

r_ASPECT <- fit3.ran.effect$LEMMA[, , 2] %>%
  as_tibble() %>%
  rownames_to_column(var = "LEMMA") %>%
  mutate(Variable = "sd(ASPECT)")

r_AGENT.STATUS <- fit3.ran.effect$LEMMA[, , 3] %>%
  as_tibble() %>%
  rownames_to_column(var = "LEMMA") %>%
  mutate(Variable = "sd(AGENT.STATUS)")

r_SUBJECT.ANIMACY <- fit3.ran.effect$LEMMA[, , 4] %>%
  as_tibble() %>%
  rownames_to_column(var = "LEMMA") %>%
  mutate(Variable = "sd(SUBJECT.ANIMACY)")



r_Intercept %>%
  bind_rows(r_ASPECT) %>%
  bind_rows(r_AGENT.STATUS) %>%
  bind_rows(r_SUBJECT.ANIMACY) %>%
  mutate(Contain_Zero = if_else(Q2.5*Q97.5 > 0, "no", "yes")) %>%
  ggplot(aes(x = LEMMA, y = Estimate, col = Contain_Zero)) +
  geom_point() +
  geom_errorbar(aes(ymin=Q2.5, ymax=Q97.5)) +
  facet_grid(. ~ Variable, scale = "free") +
  coord_flip() +
  theme(legend.position = "top") +
  theme(text=element_text(size=7, family = "Brill")) 
```
